...
export const OPENAI_API_KEY_STORAGE_KEY = 'bioquest_user_openai_api_key';
...
export const getChaptersOpenAI = async (
  ...
  const prompt = `
    You are an expert on educational syllabi. Your task is to provide a comprehensive list of all Biology chapters for a specific curriculum.
    The list should be accurate and complete according to the official syllabus.
    ...
    **Criteria:**
    ...
    Return ONLY a single valid JSON object with a single key "chapters", which contains an array of strings. Each string in the array should be a chapter name.
