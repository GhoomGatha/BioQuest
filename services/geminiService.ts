...
async function generateWithRetry(
  ...
  while (attempt < maxRetries) {
    ...
    try {
      ...
      const statusText = (errorDetails?.status || '').toLowerCase();
      const messageText = (errorDetails?.message || error?.message || '').toLowerCase();
...
`;
  ...
  throw new Error("AI generation failed with all available keys. Please check your keys in Settings and your connection.");
};

const CHAPTERS_CACHE_KEY = 'bioquest_chapters_cache_v1';
...
export const getChaptersAI = async (
  ...
  try {
    const { data: dbData, error: dbError } = await supabase.from('chapters').select('chapters_list, updated_at').eq('board', board).eq('class', classNum).eq('lang', lang).single();
    ...
    for (const provider of providers) {
        try {
            ...
            if (provider.type === 'gemini') {
                ...
                const prompt = `You are an expert on educational syllabi... Return ONLY a single valid JSON array of strings... Criteria: - Educational Board: ${board}\n- Class: ${classNum}\n${semesterInstruction}\nLanguage: ${languageMap[lang]}`;
                ...
                const result = await withTimeout(getChaptersOpenAI(board, classNum, lang, provider.key, semester), AI_OPERATION_TIMEOUT, "Chapter List Generation (OpenAI)");
  ...
  if (chaptersFromAI && chaptersFromAI.length > 0) {
    ...
    supabase.from('chapters').upsert({ board: board, class: classNum, lang: lang, chapters_list: chaptersFromAI, updated_at: new Date().toISOString() }, { onConflict: 'board, class, lang' }).then(({ error: upsertError }) => { if (upsertError) { console.warn("Failed to cache chapters in Supabase:", upsertError); } });
...
const executeWithFallbacks = async <T>(
    ...
    throw new Error(`${featureName} failed with all available keys. Please check your keys in Settings.`);
};
...
export const extractQuestionsFromPdfAI = (pdfDataUrl: string, classNum: number, lang: Language, userApiKey?: string, userOpenApiKey?: string, signal?: AbortSignal): Promise<Partial<Question>[]> => {
    const providers = createProviderList(userApiKey, userOpenApiKey);
    return executeWithFallbacks(
        providers,
        async (ai, sig) => {
            ...
            const prompt = `Extract all distinct questions from the PDF of an exam paper for Class ${classNum} in ${languageMap[lang]}. The PDF may have multiple pages. Return a valid JSON array of objects, each with "text" (string) and optional "marks" (number).`;
            const responseSchema = { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { text: { type: Type.STRING }, marks: { type: Type.NUMBER } }, required: ["text"] } };
        ...
        (apiKey, sig) => extractQuestionsFromPdfAIOpenAI(pdfDataUrl, classNum, lang, apiKey, sig),
