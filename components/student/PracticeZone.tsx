...
import { generateFlashcardsAI, generateQuestionsAI, getChaptersAI, extractQuestionsFromImageAI, suggestDiagramsAI, gradeDiagramAI, answerDoubtAI, generateStudyGuideAI } from '../../services/geminiService';
...
const PracticeZone: React.FC<PracticeZoneProps> = ({ allQuestions, lang, onStartPractice, showToast, userApiKey, userOpenApiKey }) => {
    ...
    const fetchChapters = useCallback(async (selectedBoard: string, selectedClass: number) => {
        setLoadingChapters(true);
        const resetFields = [setChapters, setCustomChapter, setMcqChapter, setStudyChapter, setDiagramChapter];
        resetFields.forEach(f => f([] as any));
        try {
            const chapterList = await getChaptersAI(selectedBoard, selectedClass, lang, undefined, userApiKey, userOpenApiKey);
            setChapters(chapterList);
        } catch (error) {
            showToast("Could not fetch chapters.", 'error');
        } finally {
            setLoadingChapters(false);
        }
    }, [lang, showToast, userApiKey, userOpenApiKey]);

    useEffect(() => {
        fetchChapters(board, studentClass);
    }, [board, studentClass, lang, fetchChapters]);
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                     <input list="chapters-datalist" value={diagramChapter} onChange={e => setDiagramChapter(e.target.value)} className={`${inputStyles} flex-grow`}
                        placeholder={loadingChapters ? t('loadingChapters', lang) : t('selectOrTypeChapter', lang)} disabled={loadingChapters}/>
