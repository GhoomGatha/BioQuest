...
import { Question, Paper, Tab, Language, Profile, QuestionSource, Difficulty, Semester } from '../types';
...
const TeacherApp: React.FC<TeacherAppProps> = ({ showToast }) => {
  ...
  const handleUploadPaper = async (paper: Paper, file: File, options: { signal: AbortSignal }) => {
    try {
        if (!session?.user) {
        ...
        if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
            throw new Error(`File is too large (max ${MAX_FILE_SIZE_MB}MB). Please upload a smaller file.`);
        }
        
        const { signal } = options;

        // Step 1: Read file to data URL (cancellable)
        const dataUrl = await readFileAsDataURL(file, signal);
        if (signal.aborted) throw new DOMException('Aborted', 'AbortError');

        // Step 2: AI Processing (cancellable)
        showToast('Scanning paper with AI... This may take a moment.', 'success');
        let extractedQuestions: Partial<Question>[] = [];
        if (file.type.startsWith('image/')) {
            extractedQuestions = await extractQuestionsFromImageAI(dataUrl, paper.class, lang, userApiKey, userOpenApiKey, signal);
        } else if (file.type === 'application/pdf') {
            extractedQuestions = await extractQuestionsFromPdfAI(dataUrl, paper.class, lang, userApiKey, userOpenApiKey, signal);
        } else {
        ...
        // Step 3: Save extracted questions to DB
        ...
        if (extractedQuestions.length > 0) {
            ...
            showToast(`${savedQuestions.length} questions extracted!`, 'success');
        } else {
            showToast(`No questions could be extracted from the file.`, 'error');
        }

        // Step 4: Upload original file to storage (not cancellable, but should be fast)
        const storageFilePath = `${session.user.id}/${paper.id}/${file.name}`;
        ...
        // Step 5: Save paper metadata to DB
        const paperToInsert = { ...paper, user_id: session.user.id, questions: savedQuestions, data_url: publicUrl };
        ...
        // Propagate error to the calling component so UI can reset
        throw error;
    }
  };
